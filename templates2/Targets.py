...
class ModeledData(object):
    ...
    def __init__(self, obsx, ref):
        ...
        # insert Forward model in MyOwnTarget
        elif ref in hv_targets:
            from myfwd import MyForwardModel
            self.plugin = MyForwardModel(obsx, ref)
...
class MyOwnTarget(SingleTarget):
    ...
    def __init__(self, x, y, yerr=None):
        ...
        ## Forward your custom plugin
        #self.moddata.plugin = MyForwardModel(obsx=x, ref=ref)
        ##self.moddata.xlabel = 'xvalues in unit'
        #self.moddata.xlabel = 'frequency (Hz)'
